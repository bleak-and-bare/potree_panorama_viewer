...
async function getPanoramas(): Promise<Panorama[]> {
    let res = await fetch("/datas/" + import.meta.env.VITE_PANO_LIST)
    ...
    for (const info of infoList) {
        ...
        try {
            res = await fetch(`/datas/${import.meta.env.VITE_PANO_INFOS}/${info}`)
            const pano: RawPanorama = await res.json()
            const panoId = info.split('-')[0]
            const panoImg = `/datas/${import.meta.env.VITE_PANO_IMAGES}/${panoId}.jpg`
